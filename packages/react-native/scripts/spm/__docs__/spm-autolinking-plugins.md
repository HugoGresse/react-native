...
manifests. On the next build a re-sync is triggered when a watched **file** is
newer than the last sync, a watched **dir** has a newer child, or a watched path
has **vanished** (a rename forces a re-sync so the config error surfaces).
...
is dropped with a warning. Absolute-only, because the generated script tests
